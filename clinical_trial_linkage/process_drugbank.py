...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Process Drugbank data')
    ...
    orange_book = pd.read_csv(os.path.join(args.save_path, 'FDA/FDA_new/products.txt'), sep='~').astype(str)

    with zipfile.ZipFile(os.path.join(args.save_path, 'drug_bank/drug-ndc-0001-of-0001.json.zip')) as zf:
        with zf.open('drug-ndc-0001-of-0001.json') as f:
            data = json.load(f)
    product = pd.DataFrame(data['results'])

    with zipfile.ZipFile(os.path.join(args.save_path, 'drug_bank/drugbank_all_full_database.xml.zip')) as zf:
    ...
    drug_names.to_csv(os.path.join(args.save_path, 'drug_bank/processed_drug_names_all.csv'), index=False)
    ...
    drug_names.to_csv(os.path.join(args.save_path, 'drug_bank/processed_drug_names_in_orangebook.csv'), index=False)
